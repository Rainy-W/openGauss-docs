...
**参数说明：**启用SSL连接。请在使用这个选项之前阅读[使用gsql连接](../GettingStarted/使用gsql访问openGauss.md)。

该参数属于POSTMASTER类型参数，请参考[表1](../Administration/参数设置.md#zh-cn_topic_0283137176_zh-cn_topic_0237121562_zh-cn_topic_0059777490_t91a6f212010f4503b24d7943aed6d846)中对应设置方法进行设置。
...
**参数说明：**设置服务器端是否强制要求SSL连接，该参数只有当参数[ssl](#zh-cn_topic_0283137371_zh-cn_topic_0237124696_zh-cn_topic_0059778664_s8c4647db116f44c4b9ce3dceee3d6ffa)为on时才有效。请在使用这个选项之前阅读[使用gsql连接](../GettingStarted/使用gsql访问openGauss.md)。
...
**取值范围：**字符串，如果指定多个加密算法，加密算法之间需要以分号分割。详细请参见[用SSL进行安全的TCP/IP连接](../Administration/用SSL进行安全的TCP-IP连接.md)获取支持的加密算法。
...
**参数说明：**指定Kerberos服务主配置文件的位置，详细请参见[配置客户端接入认证](../Administration/配置客户端接入认证.md)，仅sysadmin用户可以访问。
...
**参数说明：**设置Kerberos服务名，详细请参见[配置客户端接入认证](../Administration/配置客户端接入认证.md)。
...
**参数说明：**在使用CREATE ROLE/USER或者ALTER ROLE/USER命令创建或者修改openGauss帐户时，该参数决定是否进行密码复杂度检查。关于密码复杂度检查策略请参见[设置密码安全策略](../Administration/设置密码安全策略.md)。
...
**参数说明：**在使用ALTER USER或者ALTER ROLE修改用户密码时，该参数指定是否对新密码进行可重用天数检查。关于密码可重用策略请参见[设置密码安全策略](../Administration/设置密码安全策略.md)。
...
**参数说明：**在使用ALTER USER或者ALTER ROLE修改用户密码时，该参数指定是否对新密码进行可重用次数检查，仅sysadmin用户可以访问。关于密码可重用策略请参见[设置密码安全策略](../Administration/设置密码安全策略.md)。
...
**参数说明：**该参数指定帐户被锁定后自动解锁的时间。关于帐户自动锁定策略请参见[设置密码安全策略](../Administration/设置密码安全策略.md)。
...
**参数说明：**在任意时候，如果输入密码错误的次数达到failed\_login\_attempts则当前帐户被锁定，password\_lock\_time秒后被自动解锁，仅sysadmin用户可以访问。例如，登录时输入密码失败，ALTER USER时修改密码失败等。关于帐户自动锁定策略请参见[设置密码安全策略](../Administration/设置密码安全策略.md)。
