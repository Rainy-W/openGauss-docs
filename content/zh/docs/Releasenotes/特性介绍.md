...
-   资源池化
    提供主备机共享一份存储的能力，提供一种新的HA部署形态，解决传统HA部署下教单机存储容量翻倍的问题，同时提供备机实时一致性读能力。该特性主要依赖两个自研的公共组件：
    ...
    资源池化可选通过OCK RDMA降低DMS主备页面交换时延。TCP下的备机一致性读进行时延对比，开启OCK RDMA，备机一致性读时延至少要降低20%。
    资源池化可选通过OCK SCRLock降低获取分布式锁的时延。开启OCK SCRLock，获取分布式锁的远端时延降低6倍+。

-   CM部署和数据库部署解耦，CM支持增量升级
    支持在已有数据库进程时，CM解耦化安装，同时支持CM组件单独升级，增强数据库集群可靠性。

-   MySQL兼容性增强
    3.1.1版本新增对以下语法点的支持（只列举部分典型语法，详情请参见《数据迁移》中“MySQL兼容性说明”章节）：
    -   操作符：!、->、->>。
