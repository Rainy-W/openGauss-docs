...
-   DEPENDENCY\_ExtensionReference \(e\)：依赖对象是被依赖对象ExtensionReference的一个成员（请参见[PG\_ExtensionReference](PG_ExtensionReference.md)）。依赖对象只可以通过在被依赖对象上DROP ExtensionReference删除。函数上这个依赖类型和内部依赖一样动作，但是它为了清晰和简化gs\_dump保持分开。
